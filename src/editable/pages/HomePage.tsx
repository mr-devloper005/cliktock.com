...
export default async function HomePage() {
  const primaryTask = (SITE_CONFIG.tasks.find((task) => task.enabled && task.key === 'sbm')?.key || SITE_CONFIG.tasks.find((task) => task.enabled)?.key || 'sbm') as TaskKey
      ...
      <div className="bg-[#070b14]">
        <div className="mx-auto max-w-6xl px-4 py-6">
          <Ads
            ...
            fallback={<Ads slot="feature" showLabel eager className="mx-auto w-full" />}
          />
        </div>
      </div>
